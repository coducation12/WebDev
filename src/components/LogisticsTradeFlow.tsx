...
function NodeIcon({ type }: { type: string }) {
  const s: React.SVGProps<SVGPathElement> = {
    fill: "none", stroke: "rgba(255,255,255,0.85)", strokeWidth: 1.4,
    strokeLinecap: "round", strokeLinejoin: "round",
  };
  const ls: React.SVGProps<SVGLineElement> = { ...s } as any;
    ...
    <div className={`relative w-full ${className}`}>
      <svg viewBox="0 0 1200 600" className="w-full h-auto">
        <defs>
          ...
          <filter id="nodeGlow"><feGaussianBlur stdDeviation="8" result="b" /><feMerge><feMergeNode in="b" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
          <radialGradient id="hubGrad" cx="50%" cy="50%"><stop offset="0%" stopColor={accentColor} stopOpacity="0.3" /><stop offset="100%" stopColor={accentColor} stopOpacity="0.03" /></radialGradient>
        </defs>
        ...
        {/* Connection paths */}
        {PATHS.map(p => <path key={p.id} id={p.id} d={p.d} fill="none" stroke={accentLight} strokeWidth="1.2" opacity="0.12" />)}

        {/* Flowing dots */}
        {PATHS.map(p => [0, 1, 2].map(i => (
          <circle key={`${p.id}-d${i}`} r={3 - i * 0.4} fill={accentLight} opacity={0.9 - i * 0.2} filter="url(#dotGlow)">
            <animateMotion dur={`${2.6 + i * 0.5}s`} repeatCount="indefinite" begin={`${i * 0.8}s`}><mpath href={`#${p.id}`} /></animateMotion>
          </circle>
        )))}
